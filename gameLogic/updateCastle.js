function updateCastleInteriorPosition() {
  let speed = 5;
  let newX = playerX;
  let newY = playerY;
  ...
  if (isMoving) {
    ...
    if (shouldMove) {
      ...
      playerX = constrain(playerX, 0, castleInterior.width - 75);
      playerY = constrain(playerY, 0, castleInterior.height - 75);

      // Exit door back to world
      if (collides(playerX, playerY, 75, 75,
                   bottomDoor.x, bottomDoor.y, bottomDoor.w, bottomDoor.h)
          && !isTransitioning) {
