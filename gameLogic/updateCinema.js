...
function updateCinemaInteriorPosition() {
    ...
    if (isMoving) {
        ...
        if (shouldMove) {
            ...
            // Check collisions with NPCs, excluding Maxo, Giomari, Ermalo, and Joniko to allow pop-up triggers
            for (let obstacle of obstacles) {
                if ([].includes(obstacle.type)) { // Exclude all NPCs: 'joniko', 'maxo', 'giomari', 'ermalo'
            ...
            if (canMove) {
                ...
                // Exit cinema when stepping on bottom door
                if (collides(playerX, playerY, 75, 75,
                             cinemaBottomDoor.x, cinemaBottomDoor.y,
                             cinemaBottomDoor.w, cinemaBottomDoor.h)
                    && !isTransitioning) {
                    isTransitioning = true;
                    gameState = 'game';
                    // Place just outside cinema door to avoid immediate re-entry
                    playerX = cinemaDoor.x + cinemaDoor.w / 2;
                    playerY = cinemaDoor.y + cinemaDoor.h + 20;
