...
function drawSchoolInteriorMiniMap() {
  try {
    let miniMapWidth = 160;
    let miniMapHeight = 160;
    let scaleX = miniMapWidth / schoolInterior.width;
    let scaleY = miniMapHeight / schoolInterior.height;
    ...
    console.log('School Minimap Variables:', {
      schoolBottomDoor: schoolBottomDoor,
      playerX: playerX,
      playerY: playerY,
      scaleX: scaleX,
      scaleY: scaleY
    ...
    text('School Minimap', miniMapX + miniMapWidth / 2, miniMapY - 20);
    ...
    if (schoolBottomDoor && typeof schoolBottomDoor.x !== 'undefined') {
      fill(255, 255, 255, minimapAlpha);
      rect(miniMapX + schoolBottomDoor.x * scaleX,
           miniMapY + schoolBottomDoor.y * scaleY,
           schoolBottomDoor.w * scaleX,
           schoolBottomDoor.h * scaleY);
    } else {
      console.warn('schoolBottomDoor variables undefined in drawSchoolInteriorMiniMap');
    }
    ...
    console.error('Error in drawSchoolInteriorMiniMap:', error);
  } finally {
    pop();
  }
}
...
window.drawSchoolInteriorMiniMap = drawSchoolInteriorMiniMap;